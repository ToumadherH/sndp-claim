...
import ReclamationsTableAssistant from "../components/ReclamationsTableAssistant";
import logoAgil from "../assets/logoagil.png";
import Notifications from "../components/Notifications";

interface AssistantProfile {
  name: string;
  _id: string; // on aura besoin pour les requêtes
}
  ...
  const fetchProfile = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:8000/api/assistant/profile",
        {
      ...
      console.error("Failed to fetch assistant profile:", err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-700 text-white flex flex-col">
      ...
      <main className="flex-1 p-6">
        <h1 className="text-5xl font-bold">Assistant Dashboard</h1>
        <Notifications/>
        ...
        {profile && <ReclamationsTableAssistant assistantId={profile._id} />}
